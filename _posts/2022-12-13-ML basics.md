...
monotonically decreasing, if we can find a $N_i$ making $f'_{N,1}(N_i)=f'_{N,2}(N_i)$, then $\lim_{N \to +\infty} f_{N}=0$ (Note: $f^{(2)}_{N,1} \geq 0, f^{(2)}_{N,2} >0$). We then define <b>break point</b> $k$, and $k=N_i$.