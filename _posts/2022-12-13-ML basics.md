...
Then:<br />
$$\squrt{frac{8}{N}ln(frac{4(2N)^{d_{vc}}}{\delt})}=\epsilon$$
by replace $\epsilon with (8)$:
$$|E_{in}(h)-E_{out}(h)|> \squrt{frac{8}{N}ln(frac{4(2N)^{d_{vc}}}{\delta})} $$
Then, we get:
$$E_{in}(h)-\squrt{frac{8}{N}ln(frac{4(2N)^{d_{vc}}}{\delta})}  \leg E_{out}(h) \leg E_{in}(h)+ \underbrace{\squrt{frac{8}{N}ln(frac{4(2N)^{d_{vc}}}{\delta})}}_{\Omega(\delta,N,\mathbb{H})}$$
