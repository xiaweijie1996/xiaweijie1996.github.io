...
<br />
<b>Break point</b><br />
Untile now, we have not solve the problem, becase the $f_{N}=2m_{\mathcal{H}}(N)exp(-2\epsilon^2N)$ is a function of $N$, and we do know not its value. However, since the $f_{N,1}=2m_{\mathcal{H}}(N)$ is monotonically increasing and $f_{N,2}=exp(-2\epsilon^2N)$
monotonically decreasing, if we can find a $N_i$ making $f'_{N,1}=f'_{N,2}$, then $\lim_{N \to +\infty} f_{N}=0$ (Note: f^{(2)}_{N,1} \geq 0, f^{(2)}_{N,2} >0). We then define <b>break point</b> k, and $k=N_i$ 