...
In fact, by statistical derivation, we can get <b>Vapnik-Chervonenkis (VC) Bound</b> function:
$$\mathbb{P}[{\forall}h \in \mathcal{H} s.t. |E_{in}(h)-E_{out}(h)|>\epsilon] \leq 4m_{\mathcal{H}}(2N)exp(-\frac{1}{8}\epsilon^2N)$$
$$m_{\mathcal{H}}(2N) \leq N^{K-1}$$
