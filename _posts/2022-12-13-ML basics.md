...
<table border="1">
    <tr>
        <td>**<font face="黑体" size=6>Symbol</font>**</td>
        <td>**<font face="黑体" size=6></font>**</td>
    </tr>
...
error of $g$ in the training dataset, $E_{out}(h)$ represent the error of $g$ out of the training dataset $x_{out} = \mathcal{X}-x_{in}$. Then, If a machine can learn, the following conditions must be satified.\
*$E_{in}(h) \approx 0 $, this means that $g$ performs good enough in training dataset. \
*$E_{in}(h) \approx $E_{out}(h) $, this means that $g$ have generality can be expended to the whole dataset. \
